####
# Matching Algorithms with Replacement - CROSS-CITY VERSION:
# Robustness check
# This script runs propensity score matching WITH replacement across cities
# Key difference: antiexact = "cbsa_title" and no county_id exact matching
####
...
# output directory
variant <- "no_cbsa"  # cross-city matching variant
...
# Use exact matching variables for no_cbsa variant
exact_matching_vars <- exact_matching_vars_no_cbsa
...
# Create unstacked versions (one observation per GISJOIN_1950 and year) -----
cat("\n=== CREATING UNSTACKED DATASETS ===\n")

# No-stack 1-year version: drop match identifiers, keep one obs per tract-year
tract_data_matched_no_stack_1_year <- tract_data_matched_1_year %>%
  select(-match_group, -group_type, -matched_treatment_year, -weights) %>%
  distinct(GISJOIN_1950, year, .keep_all = TRUE)

# No-stack 2-year version
tract_data_matched_no_stack_2_year <- tract_data_matched_2_year %>%
  select(-match_group, -group_type, -matched_treatment_year, -weights) %>%
  distinct(GISJOIN_1950, year, .keep_all = TRUE)

# Save no-stack datasets
write_csv(tract_data_matched_no_stack_1_year,
          here(output_data_dir, "tract_data_matched_no_stack_1_year.csv"))

write_csv(tract_data_matched_no_stack_2_year,
          here(output_data_dir, "tract_data_matched_no_stack_2_year.csv"))

cat("No-stack datasets saved to:\n",
    here(output_data_dir, "tract_data_matched_no_stack_1_year.csv"), "\n",
    here(output_data_dir, "tract_data_matched_no_stack_2_year.csv"), "\n")

cat("No-stack dataset dimensions:\n")
cat("1-year: ", nrow(tract_data_matched_no_stack_1_year), " observations, ",
    n_distinct(tract_data_matched_no_stack_1_year$GISJOIN_1950), " unique tracts\n")
cat("2-year: ", nrow(tract_data_matched_no_stack_2_year), " observations, ",
    n_distinct(tract_data_matched_no_stack_2_year$GISJOIN_1950), " unique tracts\n")
...
if (nrow(unmatched_tracts) > 0) {
  ...
  cat("\n1. City distribution of unmatched tracts:\n")
  city_summary <- unmatched_tracts %>%
  ...
  exact_combo_summary <- unmatched_tracts %>%
    count(redlined_binary_80pp, ur_binary_5pp, cbsa_title, sort = TRUE)
