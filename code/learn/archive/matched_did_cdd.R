...
perform_matching_for_year <- function(data, treatment_year, match_vars, nearest_neighbors) {
  ...
  m.out <- matchit(match_formula,
                   data = matching_data, 
                   exact = "city", 
                   method = "nearest", ratio = nearest_neighbors,
                   #add caliper 
                   caliper = 0.2,
                   std.caliper = TRUE)
...
for (year in treatment_years) {
  ...
  matched_datasets[[df_name]] <- 
    perform_matching_for_year(census_tract_data, year, matching_vars, 3)
}
...
covariates <- c("black_share", "white_share",  "asinh_median_income",
                "asinh_median_home_value", "asinh_median_rent", "distance_from_cbd",
                "population_density", "total_pop", "black_pop",
...
did_estimation <- function(data, outcome_var) {
  ...
  formula <- as.formula(paste(outcome_var, "~ treated + post + did | year + match_group"))
