...
did_output_dir <- "output/regression_results/stacked_did/cdd"
...
treated_tracts_panel <-
  st_read("data/derived/merged/treated_tracts_panel_cdd.gpkg") %>% 
  st_drop_geometry()

# read in Census trat sample with treatment status
census_tract_sample <-
  st_read("data/derived/merged/census_tract_sample_with_treatment_status_cdd.gpkg")
...
write_csv(event_study_data_rings, "data/derived/merged/event_study_data_rings.csv")
...
run_event_study_regression <- function(data, dep_var) {
  ...
  formula <- as.formula(paste(dep_var, "~ i(location_type_factor,event_time)",
                               "| treated_id^year + tract_id + treated_id^location_type_dummy"))
  ...
  # Run the regression
  model <- feols(formula, data = data, cluster = c("treated_id", "tract_id"))
...
create_event_study_plot <- function(reg_results_df, data, dep_var, title) {
  ...
  # create the plot
  ggplot(reg_results_df$coefficients, aes(x = event_time, y = estimate, color = location_type)) +
    ...
    labs(title = title,
         ...
         caption = paste("Treated coefficient at t = 10 is", round(t10_coefficient, 2),
                         "of the treated tract mean at t = -10\nn =", n_obs)) +
...
##### create histograms of variables of interest, by treated and control groups ----

# treated
ggplot(event_study_data_rings %>% filter(location_type == "treated"), aes(x = total_pop)) +
  geom_histogram(bins = 30, alpha = 0.5, position = "identity") +
  labs(title = "Histogram of Total Population by Location Type",
       x = "Total Population (asinh)",
       y = "Frequency") +
  theme_minimal()

# inner
ggplot(event_study_data_rings %>% filter(location_type == "inner"), aes(x = total_pop)) +
  geom_histogram(bins = 30, alpha = 0.5, position = "identity") +
  labs(title = "Histogram of Total Population by Location Type",
       x = "Total Population (asinh)",
       y = "Frequency") +
...
# outer
ggplot(event_study_data_rings %>% filter(location_type == "outer"), aes(x = total_pop)) +
  geom_histogram(bins = 30, alpha = 0.5, position = "identity") +
  labs(title = "Histogram of Total Population by Location Type",
       x = "Total Population (asinh)",
       y = "Frequency") +
...
blk_sh_25p <- quantile(y$black_share, probs = 0.25)
blk_sh_50p <- quantile(y$black_share,  probs = 0.5)
blk_sh_75p <- quantile(y$black_share, probs = 0.75)
