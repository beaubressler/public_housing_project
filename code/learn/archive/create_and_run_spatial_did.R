...
# choices are "digitized", "cdd_large", or "cdd_small"
data_type <- "digitized"
...
# output paths
# regression results
did_output_dir <- paste0("output/regression_results/stacked_did/", data_type, "/")

# event study data paths
event_study_rings_output_path <- paste0("data/derived/merged/event_study_data_", data_type, ".csv")
unique_tracts_and_rings_output_path <- paste0("data/derived/merged/unique_tracts_and_rings_", data_type, "csv")

#input paths 
treated_tracts_panel_path <- paste0("data/derived/merged/treated_tracts_panel_", data_type, ".gpkg")
census_tract_sample_path <- paste0("data/derived/merged/census_tract_sample_with_treatment_status_", data_type, ".gpkg")
...
determine_clusters <- function(n_tracts, factor = 0.3, min_clusters = 2, max_clusters = 20) {
...
unique_tracts_and_rings <- 
  event_study_data_rings %>% 
  select(STATE, COUNTY, TRACTA, location_type) %>% 
  distinct() 
...
# Create balance table for treated, inner, and outer rings ----

## get data for balance test: Data in event time = -10
balanced_data <- event_study_data_rings %>% filter(event_time == -10)
...
balance_table <- CreateTableOne(vars = covariates,
                                ...
                                data = balanced_data %>% filter(city == "New York City"),
                                test = TRUE)
