...
# read data
public_housing_projects <- 
  read_csv("data/derived/public_housing/working/geocoded_projects.csv")

## geolocation ----
# Get longitude and latitude from NGDA and PSH 
public_housing_projects_geolocated <-
  public_housing_projects %>% 
  # drop if missing latitude or longitude
  filter(!is.na(lat), !is.na(long))

# convert to sf object
public_housing_projects_geolocated_sf <- 
  public_housing_projects_geolocated %>% 
...
public_housing_projects_geolocated_sf <- 
  st_transform(public_housing_projects_geolocated_sf, st_crs(census_tract_sample))

st_crs(public_housing_projects_geolocated_sf)

# checking something
# y <- cdd_project_sample %>% 
#   group_by(project_code) %>% 
#   # keep first for each project code
#   slice(1) %>%  
#   ungroup() %>% 
#   filter(!str_detect(name, "Scattered"))
# 
# # number of rows by locality in y
# y %>% 
#   st_drop_geometry() %>%
#   count(locality) 
...
treated_tracts <-
  treated_tracts %>% 
  group_by(STATE, COUNTY, TRACTA, YEAR, year_completed) %>% 
  mutate(total_public_housing_units = sum(total_public_housing_units, na.rm = TRUE)) %>%
  arrange(STATE, COUNTY, TRACTA, year_completed) %>% 
...
treated_tracts_panel <-
  ...
  #st_drop_geometry() %>% 
  select(tract_id, TRACTA, COUNTY, STATE, YEAR, treatment_year, total_public_housing_units) %>%
  mutate(treated = 1)
...
# 1.
st_write(census_tract_sample_with_treatment_status, "data/derived/merged/census_tract_sample_with_treatment_status_digitized.gpkg",
         append = FALSE, layer = "points", driver = "GPKG", overwrite = TRUE, delete_dsn = TRUE)

# 2.
st_write(public_housing_project_sample, "data/derived/public_housing/working/digitized_housing_project_sample.gpkg",
         append = FALSE, layer = "points", driver = "GPKG", overwrite = TRUE, delete_dsn = TRUE)

# 3.
st_write(treated_tracts_panel, "data/derived/merged/treated_tracts_panel_digitized.gpkg",
         append = FALSE, layer = "points", driver = "GPKG", overwrite = TRUE, delete_dsn = TRUE)
