####
# Collapse geocoded 1930 Full Count addresses directly to 1950 tract level
#
...
# Load Saavedra and Twinam occupation scores for LIDO income calculation
st_occscores <- read_dta(here(saavedra_twinam_dir, "lido_score_1950_public_use.dta")) %>%
  rename_with(toupper)
setDT(st_occscores)

message("=== Geocoded 1930 Census to Tract Aggregation ===\n")

message("Step 1: Loading geocoded address data...")

# Read geocoded addresses with quality filters
geocoded_1930 <- fread(here(geocoded_dir, "grf_unique_addresses_1930_Geocoded_ExportTable.csv")) %>%
  as_tibble() %>%
  filter(
...
message(sprintf("Loaded %d geocoded addresses (after quality filtering)", nrow(geocoded_1930)))

# Convert to spatial points
geocoded_sf <- geocoded_1930 %>%
...
grf_addresses_1930 <- fread(here(grf_dir, "grf_addresses_1930.csv")) %>%
  as_tibble() %>%
  select(histid, full_street_address)

message(sprintf("Loaded %d HISTID-address links", nrow(grf_addresses_1930)))
...
setDT(grf_addresses_1930)
setkey(grf_addresses_1930, histid)
...
cb_function_geocoded_tract <- function(x, pos) {
  ...
  x <- merge(x, grf_addresses_1930, by.x = "HISTID", by.y = "histid", all.x = TRUE)
  ...
  # 1930 uses literacy (LIT) instead of education (EDUC)
  # LIT==4 means literate
  x[age_25_plus == TRUE & !is.na(LIT) & LIT != 0, `:=`(
    literate = ifelse(LIT == 4, 1, 0)
  )]
  ...
  pop_counts <- x[, .(
    ...
    literate_pop = sum(literate, na.rm = TRUE),
    total_literacy_pop = sum(age_25_plus, na.rm = TRUE)
  ), by = GISJOIN]

  # Rent groups (household heads only)
  # 1930 uses RENT30 variable instead of RENT
  rent_data <- x[is_hh_head == TRUE & !is.na(RENT30) & RENT30 > 0 & RENT30 < 9998]
  if (nrow(rent_data) > 0) {
    rent_data[, rent_group := fcase(
      RENT30 <= 5, "rent_group_1",
      RENT30 <= 6, "rent_group_2",
      RENT30 <= 9, "rent_group_3",
      RENT30 <= 14, "rent_group_4",
      RENT30 <= 19, "rent_group_5",
      RENT30 <= 24, "rent_group_6",
      RENT30 <= 29, "rent_group_7",
      RENT30 <= 39, "rent_group_8",
      RENT30 <= 49, "rent_group_9",
      RENT30 <= 59, "rent_group_10",
      RENT30 <= 74, "rent_group_11",
      RENT30 <= 99, "rent_group_12",
      RENT30 <= 149, "rent_group_13",
      RENT30 <= 199, "rent_group_14",
  ...
  # LIDO occupation-based income (1930 uses Saavedra & Twinam scores)
  # Merge with occupation scores
  x_with_lido <- merge(x, st_occscores, by = c("STATEFIP", "SEX", "AGE", "RACE", "OCC1950", "IND1950"), all.x = TRUE)

  # Filter for individuals with valid LIDO scores
  income_data <- x_with_lido[!is.na(LIDO)]

  if (nrow(income_data) > 0) {
    # Calculate family LIDO-based income by summing LIDO scores within each family
    family_income <- income_data[, .(family_wage_raw = sum(LIDO)), by = .(GISJOIN, SERIAL)]

    # Convert LIDO scores to actual 1950 dollars
    family_income[, family_wage := family_wage_raw * 100]
...
tract_data_final <- tract_data_final %>%
  ...
  mutate(
    ...
    literacy_rate = literate_pop / total_literacy_pop,
...
st_write(tract_data_final,
         here(output_dir, "tract_data_1930_geocoded.gpkg"),
         append = FALSE, driver = "GPKG")

# Save CSV version
tract_data_final %>%
  st_drop_geometry() %>%
  write_csv(here(output_dir, "tract_data_1930_geocoded.csv"))

message(sprintf("\n=== COMPLETE ==="))
message(sprintf("Geocoded tract data saved to:"))
message(sprintf("  - %s", here(output_dir, "tract_data_1930_geocoded.gpkg")))
message(sprintf("  - %s", here(output_dir, "tract_data_1930_geocoded.csv")))
